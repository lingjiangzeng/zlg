<template>
	<div class="callenter">
		...
		<div class="leftcontentbox">
			<div class="callStatus" v-if="callwindow.ifshow==1 || callwindow.ifshow==3">
				<span class="title"><img src="../../assets/callicon.png"></img><span>等待接通中</span></span>
			</div>
			<div class="callStatus1" v-if="callwindow.ifshow==2">
				<div class="callDuration"><span class="title">通话中</span>&ensp;<span class="time">{{callwindow.timestr}}</span></div>
				<div class="userInfo">
					<img :src="callwindow.imgurl"></img>
					<div class="name_callNo">
						<span class="name" :title="name">{{callwindow.username}}</span>
						<span class="phonenumber">{{callwindow.phone}}</span>
					</div>
				</div>
			</div>
			<div class="calllist">
				<div class="title"><span class="text">今日通话 <span class="number">{{todaycall.length}}</span></span></div>
				<div class="list">
					<div :class="{usermsg:true,usermsgactive:items.active}" v-for="(items,index) in todaycall" :key="'usermsg'+index"
					 v-on:click="todaycallfun(index)">
						<div class="msg">
							<img :src="items.headPortrait"></img>
							<div class="name_callNo">
								<span class="name" :title="items.name">{{items.customerName}}</span>
								<span class="phonenumber">{{items.phone}}</span>
							</div>
						</div>
						<span class="times">{{items.beginTime.split(' ')[1].slice(0,5)}}</span>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="rightcontentox">
			...
			<div class="callDetail">
				<div class="title"><span>已购服务</span></div>
				<div class="msg">
					<div class="msgcontent" v-for="(items,index) in purchasedList" :key="'msgcontent'+index">
						<span class="title">服务{{index+1}}:</span>
						<span class="text" :title="items.purchasedServiceName">{{items.purchasedServiceName}}</span>
					</div>
				</div>
			</div>
			<div class="billorder">
				...
				<el-table ref="multipleTable" :data="customerWOInfo" tooltip-effect="dark" size="small" height="280">
					...
					<el-table-column prop="createTime" show-overflow-tooltip width="150">
...
<script>
	import callenter from './callenter.js'
	export default {
		...callenter
	}
</script>
